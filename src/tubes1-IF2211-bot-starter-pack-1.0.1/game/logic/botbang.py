...
class BotBang(BaseLogic):
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        portal_1, portal_2 = Portal.sorted_portal(current, board)
        self.passPortal = False

        timeLeft = props.milliseconds_left
        backBase = Path.calculate_move(current, props.base) * board.minimum_delay_between_moves * 10
        ...
        if inventory == 5 or (( backBase >= timeLeft - 3000 and backBase < timeLeft) 
            ...
            toBase = Path.calculate_move(current, props.base)
            ...
            if inventory < 5:
                ...
                for d in diamond:
                    ...
                    takeDiamond = Path.calculate_move(current, d.position) + Path.calculate_move(d.position, props.base)
                    distancePortal = Path.calculate_move(current, portal_1) + Path.calculate_move(portal_2, d.position)\
                                    + Path.calculate_move(d.position, props.base)
                    if inventory == 4 and point == 2:
                        ...
                        elif distancePortal < takeDiamond and toBase >= distancePortal:
                ...
                portalToBase = Path.calculate_move(current, portal_1) + Path.calculate_move(portal_2, props.base)
            ...
            diamond = board.diamonds
            distanceButton = Path.calculate_move(current, board.game_objects[2].position)
            temp = 0
            if distanceButton > 0:
                temp = (3 / 7) / distanceButton
                self.goal_position = board.game_objects[2].position
            for d in diamond:
                point = d.properties.points
                distanceDiamond = Path.calculate_move(current, d.position)
                distancePortal = (Path.calculate_move(current, portal_1) + Path.calculate_move(portal_2, d.position))
                if inventory <= 3:
                    ...
                    distanceBase = Path.calculate_move(d.position, props.base)
                    denseDiamond = point / (distanceDiamond + distanceBase)
                    densePortal = point / (distancePortal + distanceBase)
                    temp = self.destination(denseDiamond, temp, densePortal, d.position, portal_1)

        # tackle kalau ada kesempatan
        self.goal_position = Path.tackle(current, board, self.goal_position)

        # cari direction dari destination
        delta_x, delta_y = Path.direction(
            current.x, current.y,
            self.goal_position.x, self.goal_position.y
        )
        if Portal.avoid_portal(board, delta_x + current.x, delta_y + current.y) and not self.passPortal:
            if delta_x != 0:
...
class Portal:
    def avoid_portal(board: Board, x, y) -> bool:
        # koordinat portal 1
    ...
    def sorted_portal(current: Position, board: Board):
        # jarak current ke tiap portal
        portal_1 = Path.calculate_move(current, board.game_objects[0].position)
        portal_2 = Path.calculate_move(current, board.game_objects[1].position)
...
class Path:
    def calculate_move(current: Position, other: Position) -> int:
        # hitung jarak antara dua titik
        return abs(current.x - other.x) + abs(current.y - other.y)


    def direction(current_x, current_y, dest_x, dest_y):
